...
export function Header() {
    ...
    return (
        <header className="flex gap-10 lg:gap-20 justify-between p-2 mb-6 mx-4">
            <PageHeaderNavToggle hidden={showFullWidthSearch} />
            <form className={`gap-4 flex-grow justify-center ${showFullWidthSearch ? "flex" : "hidden md:flex"} `}>

                {showFullWidthSearch && (
                    <Button onClick={() => setShowFullWidthSearch(false)} type="button" size="icon" className="flex-shrink-0" variant="ghost"><ArrowLeft /> </Button>
                )}
...
type PageHeaderNavToggleProps = {
    hidden?: boolean
}
export function PageHeaderNavToggle({ hidden = false }: PageHeaderNavToggleProps) {
